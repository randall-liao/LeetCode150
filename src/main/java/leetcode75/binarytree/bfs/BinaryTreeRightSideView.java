...
public class BinaryTreeRightSideView {
  ...
  public List<Integer> rightSideView(TreeNode root) {
    List<List<TreeNode>> listNodeGroupByLevel = bfsTraversalByLevel(root);
    for (List<TreeNode> levelNodeList : listNodeGroupByLevel) {
      this.resultList.add(levelNodeList.get(levelNodeList.size() - 1).val);
    }
    return resultList;
  }

  private List<List<TreeNode>> bfsTraversalByLevel(TreeNode root) {
