...
class BinarySearchForthAttemptTest {

  @Test
  void search_evenInput_expectedMidIndex() {
    int[] inputArray = {1, 2, 3, 4, 5};
    int intSearchKey = 3;
    assertEquals(2, BinarySearchForthAttempt.search(inputArray, intSearchKey));
  }
  ...
  void search_evenInput_expectedLeftIndex() {
    int[] inputArray = {1, 2, 3, 4, 5};
    int intSearchKey = 2;
    assertEquals(1, BinarySearchForthAttempt.search(inputArray, intSearchKey));
  }
  ...
  void search_evenInput_expectedRightIndex() {
    int[] inputArray = {1, 2, 3, 4, 5};
    int intSearchKey = 4;
    assertEquals(3, BinarySearchForthAttempt.search(inputArray, intSearchKey));
  }
  ...
  void testEmptyArray() {
    int[] array = {};
    int key = 5;
    int result = BinarySearchForthAttempt.search(array, key);
  ...
  void testSingleElementArray() {
    ...
    assertEquals(
        0,
        BinarySearchForthAttempt.search(array, 10),
        "Searching for the only element should return its index");

    // Test non-existing element
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(array, 5),
  ...
  void testMultipleElementsArray() {
    ...
    assertEquals(0, BinarySearchForthAttempt.search(array, 1), "Should find the first element");
    assertEquals(3, BinarySearchForthAttempt.search(array, 7), "Should find the middle element");
    assertEquals(6, BinarySearchForthAttempt.search(array, 13), "Should find the last element");

    // Test non-existing elements
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(array, 2),
        "Should return -1 for a non-existing element");
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(array, 14),
  ...
  void testExtremeValues() {
    ...
    assertEquals(
        0,
        BinarySearchForthAttempt.search(array, Integer.MIN_VALUE),
        "Should find Integer.MIN_VALUE");
    assertEquals(
        4,
        BinarySearchForthAttempt.search(array, Integer.MAX_VALUE),
        "Should find Integer.MAX_VALUE");

    // Test non-existing extreme values
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(array, Integer.MIN_VALUE + 1),
        "Should return -1 for a value just above Integer.MIN_VALUE");
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(array, Integer.MAX_VALUE - 1),
  ...
  void testLargeArray() {
    ...
    assertEquals(
        0,
        BinarySearchForthAttempt.search(largeArray, 0),
        "Should find the first element in a large array");
    assertEquals(
        499_999,
        BinarySearchForthAttempt.search(largeArray, 999_998),
        "Should find a middle element in a large array");
    assertEquals(
        999_999,
        BinarySearchForthAttempt.search(largeArray, 1_999_998),
        "Should find the last element in a large array");

    // Test non-existing elements
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(largeArray, -2),
        "Should return -1 for a negative key not in the array");
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(largeArray, 1),
        "Should return -1 for an odd key not in the array");
    assertEquals(
        -1,
        BinarySearchForthAttempt.search(largeArray, 2_000_000),
